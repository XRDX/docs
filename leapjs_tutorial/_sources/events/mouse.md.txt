...
如果应用需要在手机上运行，需要特别注意事件在手机和电脑上的区别。在手机上，没有鼠标的存在，只有点击屏幕后才有移动事件的触发。

电脑：`Move` -> `Down` -> `Move` -> `Up/Click` 
...
Mouse.up 当鼠标的左键松开时触发该事件，经常配合Mouse.down使用。
